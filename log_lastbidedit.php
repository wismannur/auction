...
$log_lastbid_edit = NULL; // Initialize page object first

class clog_lastbid_edit extends clog_lastbid {
	...
	// Page object name
	var $PageObjName = 'log_lastbid_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_last_row_id")) {
	...
	function RenderRow() {
		...
		$this->last_time->ViewCustomAttributes = "";
			...
			$this->last_row_id->EditValue = $this->last_row_id->CurrentValue;
			$this->last_row_id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->last_user_id->SetDbValueDef($rsnew, $this->last_user_id->CurrentValue, NULL, $this->last_user_id->ReadOnly);

			// last_bid
			$this->last_bid->SetDbValueDef($rsnew, $this->last_bid->CurrentValue, NULL, $this->last_bid->ReadOnly);

			// last_time
			$this->last_time->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->last_time->CurrentValue, 0), NULL, $this->last_time->ReadOnly);
...
// Create page object
if (!isset($log_lastbid_edit)) $log_lastbid_edit = new clog_lastbid_edit();

// Page init
$log_lastbid_edit->Page_Init();

// Page main
$log_lastbid_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$log_lastbid_edit->Page_Render();
?>
...
var CurrentForm = flog_lastbidedit = new ew_Form("flog_lastbidedit", "edit");

// Validate form
flog_lastbidedit.Validate = function() {
...
flog_lastbidedit.Form_CustomValidate = 
...
flog_lastbidedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $log_lastbid_edit->ShowPageHeader(); ?>
<?php
$log_lastbid_edit->ShowMessage();
?>
<form name="flog_lastbidedit" id="flog_lastbidedit" class="<?php echo $log_lastbid_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($log_lastbid_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $log_lastbid_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="log_lastbid">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($log_lastbid_edit->IsModal) ?>">
<?php if (!$log_lastbid_edit->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($log_lastbid_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_log_lastbidedit" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($log_lastbid->last_row_id->Visible) { // last_row_id ?>
<?php if ($log_lastbid_edit->IsMobileOrModal) { ?>
	<div id="r_last_row_id" class="form-group">
		<label id="elh_log_lastbid_last_row_id" for="x_last_row_id" class="<?php echo $log_lastbid_edit->LeftColumnClass ?>"><?php echo $log_lastbid->last_row_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $log_lastbid_edit->RightColumnClass ?>"><div<?php echo $log_lastbid->last_row_id->CellAttributes() ?>>
<span id="el_log_lastbid_last_row_id">
<span<?php echo $log_lastbid->last_row_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $log_lastbid->last_row_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="log_lastbid" data-field="x_last_row_id" name="x_last_row_id" id="x_last_row_id" value="<?php echo ew_HtmlEncode($log_lastbid->last_row_id->CurrentValue) ?>">
...
<?php if ($log_lastbid_edit->IsMobileOrModal) { ?>
	<div id="r_last_user_id" class="form-group">
		<label id="elh_log_lastbid_last_user_id" for="x_last_user_id" class="<?php echo $log_lastbid_edit->LeftColumnClass ?>"><?php echo $log_lastbid->last_user_id->FldCaption() ?></label>
		<div class="<?php echo $log_lastbid_edit->RightColumnClass ?>"><div<?php echo $log_lastbid->last_user_id->CellAttributes() ?>>
...
<?php if ($log_lastbid_edit->IsMobileOrModal) { ?>
	<div id="r_last_bid" class="form-group">
		<label id="elh_log_lastbid_last_bid" for="x_last_bid" class="<?php echo $log_lastbid_edit->LeftColumnClass ?>"><?php echo $log_lastbid->last_bid->FldCaption() ?></label>
		<div class="<?php echo $log_lastbid_edit->RightColumnClass ?>"><div<?php echo $log_lastbid->last_bid->CellAttributes() ?>>
...
<?php if ($log_lastbid_edit->IsMobileOrModal) { ?>
	<div id="r_last_time" class="form-group">
		<label id="elh_log_lastbid_last_time" for="x_last_time" class="<?php echo $log_lastbid_edit->LeftColumnClass ?>"><?php echo $log_lastbid->last_time->FldCaption() ?></label>
		<div class="<?php echo $log_lastbid_edit->RightColumnClass ?>"><div<?php echo $log_lastbid->last_time->CellAttributes() ?>>
...
<?php } ?>
<?php if (!$log_lastbid_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $log_lastbid_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $log_lastbid_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
flog_lastbidedit.Init();
</script>
<?php
$log_lastbid_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$log_lastbid_edit->Page_Terminate();
?>
