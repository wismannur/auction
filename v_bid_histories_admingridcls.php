...
$v_bid_histories_admin_grid = NULL; // Initialize page object first

class cv_bid_histories_admin_grid extends cv_bid_histories_admin {
	...
	// Page object name
	var $PageObjName = 'v_bid_histories_admin_grid';

	// Grid form hidden field names
	var $FormName = 'fv_bid_histories_admingrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["v_bid_histories_admin"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function ClearInlineMode() {
		$this->bid_value->FormValue = ""; // Clear form value
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_grade") && $objForm->HasValue("o_grade") && $this->grade->CurrentValue <> $this->grade->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_bid_value") && $objForm->HasValue("o_bid_value") && $this->bid_value->CurrentValue <> $this->bid_value->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_CompanyName") && $objForm->HasValue("o_CompanyName") && $this->CompanyName->CurrentValue <> $this->CompanyName->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		...
		$this->bid_time->CurrentValue = NULL;
		$this->bid_time->OldValue = $this->bid_time->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->bid_value->setOldValue($objForm->GetValue("o_bid_value"));
		if (!$this->CompanyName->FldIsDetailKey) {
			$this->CompanyName->setFormValue($objForm->GetValue("x_CompanyName"));
		}
		$this->CompanyName->setOldValue($objForm->GetValue("o_CompanyName"));
	...
	function RenderRow() {
		...
		$this->CompanyName->ViewCustomAttributes = "";
			...
			if (strval($this->bid_value->EditValue) <> "" && is_numeric($this->bid_value->EditValue)) {
			$this->bid_value->EditValue = ew_FormatNumber($this->bid_value->EditValue, -2, -2, -2, -2);
			$this->bid_value->OldValue = $this->bid_value->EditValue;
			}
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "v_auction_list_admin") {
			...
			if ($GLOBALS["v_auction_list_admin"]->EventCancelled) $this->EventCancelled = TRUE;
		}
