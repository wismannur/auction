...
//
// Table class for tr_bid
//
class ctr_bid extends cTable {
	var $row_id;
	var $master_id;
	var $user_id;
	var $bid_time;
	...
	function __construct() {
		...
		$this->TableVar = 'tr_bid';
		$this->TableName = 'tr_bid';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tr_bid`";
		$this->DBID = 'DB';
		...
		// row_id
		$this->row_id = new cField('tr_bid', 'tr_bid', 'x_row_id', 'row_id', '`row_id`', '`row_id`', 3, -1, FALSE, '`row_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// master_id
		$this->master_id = new cField('tr_bid', 'tr_bid', 'x_master_id', 'master_id', '`master_id`', '`master_id`', 3, -1, FALSE, '`master_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// user_id
		$this->user_id = new cField('tr_bid', 'tr_bid', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_id->Sortable = TRUE; // Allow sort
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// bid_time
		$this->bid_time = new cField('tr_bid', 'tr_bid', 'x_bid_time', 'bid_time', '`bid_time`', ew_CastDateFieldForLike('`bid_time`', 11, "DB"), 135, 11, FALSE, '`bid_time`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->bid_time->Sortable = TRUE; // Allow sort
		...
		// bid_value
		$this->bid_value = new cField('tr_bid', 'tr_bid', 'x_bid_value', 'bid_value', '`bid_value`', '`bid_value`', 5, -1, FALSE, '`bid_value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// bid_winner
		$this->bid_winner = new cField('tr_bid', 'tr_bid', 'x_bid_winner', 'bid_winner', '`bid_winner`', '`bid_winner`', 202, -1, FALSE, '`bid_winner`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->bid_winner->Sortable = FALSE; // Allow sort
		$this->bid_winner->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->bid_winner->TrueValue = 'Y';
		$this->bid_winner->FalseValue = 'N';
		$this->bid_winner->OptionCount = 2;
		$this->fields['bid_winner'] = &$this->bid_winner;

		// bid_time_ms
		$this->bid_time_ms = new cField('tr_bid', 'tr_bid', 'x_bid_time_ms', 'bid_time_ms', '`bid_time_ms`', '`bid_time_ms`', 5, -1, FALSE, '`bid_time_ms`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->bid_time_ms->Sortable = TRUE; // Allow sort
		$this->bid_time_ms->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['bid_time_ms'] = &$this->bid_time_ms;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tr_bid`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tr_bidedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tr_bidadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tr_bidview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tr_bidview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tr_bidadd.php?" . $this->UrlParm($parm);
		else
			$url = "tr_bidadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tr_bidedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tr_bidadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tr_biddelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->bid_winner->setDbValue($rs->fields('bid_winner'));
		$this->bid_time_ms->setDbValue($rs->fields('bid_time_ms'));
	}
	...
	function RenderEditRow() {
		...
		// bid_winner
		$this->bid_winner->EditCustomAttributes = "";
		$this->bid_winner->EditValue = $this->bid_winner->Options(FALSE);

		// bid_time_ms
		$this->bid_time_ms->EditAttrs["class"] = "form-control";
		$this->bid_time_ms->EditCustomAttributes = "";
		$this->bid_time_ms->EditValue = $this->bid_time_ms->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->bid_winner->Exportable) $Doc->ExportCaption($this->bid_winner);
					if ($this->bid_time_ms->Exportable) $Doc->ExportCaption($this->bid_time_ms);
				} else {
					if ($this->user_id->Exportable) $Doc->ExportCaption($this->user_id);
					if ($this->bid_time->Exportable) $Doc->ExportCaption($this->bid_time);
					if ($this->bid_value->Exportable) $Doc->ExportCaption($this->bid_value);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->bid_winner->Exportable) $Doc->ExportField($this->bid_winner);
						if ($this->bid_time_ms->Exportable) $Doc->ExportField($this->bid_time_ms);
					} else {
						if ($this->user_id->Exportable) $Doc->ExportField($this->user_id);
						if ($this->bid_time->Exportable) $Doc->ExportField($this->bid_time);
						if ($this->bid_value->Exportable) $Doc->ExportField($this->bid_value);
